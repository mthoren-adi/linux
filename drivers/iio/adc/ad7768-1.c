			       ...
			       unsigned int *data)
{
	...
	if (len == 1)
		*data = (unsigned int)st->adc_data.reg_value;
	else
		*data = be32_to_cpu(st->adc_data.raw);
...
static int ad7768_scan_direct(struct iio_dev *indio_dev)
{
	...
	ret = ad7768_spi_reg_read(st, AD7768_REG_ADC_DATA, 3, &readval);
	if (ret < 0)
		return ret;
	/*
...
static int ad7768_setup(struct ad7768_state *st)
{
	...
	return ad7768_set_freq(st, 256000);
}
...
static irqreturn_t ad7768_trigger_handler(int irq, void *p)
{
	...
	ret = spi_read(st->spi, &st->adc_data.raw, 3);
	if (ret < 0)
		goto err_unlock;

	iio_push_to_buffers_with_timestamp(indio_dev, &st->adc_data.raw,
...
static int ad7768_buffer_postenable(struct iio_dev *indio_dev)
{
	...
	if (st->interrupt_available) {
		return ad7768_spi_reg_write(st,
...
static int ad7768_probe(struct spi_device *spi)
{
	...
	struct device_node *interrupt_node = NULL;
	...
	interrupt_node = of_parse_phandle(node, "interrupts", 0);
	st->interrupt_available = interrupt_node ? true : false;
	...
	if (st->interrupt_available) {
		dev_notice(&spi->dev, "IRQ found: using GPIO IRQ\n");
		...
		dev_notice(&spi->dev, "IRQ missing: using SPI engine\n");
