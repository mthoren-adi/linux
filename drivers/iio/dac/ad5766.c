 ...
 * @crt_scales_avail_no:Current scales available number
...
struct ad5766_state {
	...
	enum ad5766_voltage_range	crt_scales_avail_no;
...
static void ad5766_set_scale_avail(struct ad5766_state *st)
{
	...
	st->crt_scales_avail_no = 0;

	for (i = 0; i < AD5766_VOLTAGE_RANGE_MAX; i++) {
		if (st->scale_matrix[st->crt_range] & BIT(i)) {
			st->crt_scale_avail[st->crt_scales_avail_no][0] =
				st->scale_avail[i][0];
			st->crt_scale_avail[st->crt_scales_avail_no][1] =
				st->scale_avail[i][1];
			st->crt_scales_avail_no++;
		}
	}
}
...
static int ad5766_get_scale_idx(struct ad5766_state *st, int crt_idx)
{
	int i;

	for (i = 0; i < AD5766_VOLTAGE_RANGE_MAX; i++) {
		if (st->crt_scale_avail[crt_idx][0] == st->scale_avail[i][0] &&
			st->crt_scale_avail[crt_idx][1] == st->scale_avail[i][1]) {
			return i;
		}
	}
...
static int ad5766_set_scale(struct ad5766_state *st, int val, int val2)
{
	...
	for (i = 0; i < st->crt_scales_avail_no; i++) {
		if ((*tbl)[i][0] != val || (*tbl)[i][1] != val2)
			continue;

		scale_idx = ad5766_get_scale_idx(st, i);
		if (scale_idx < 0)
			return scale_idx;
			     ...
			     long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		...
		*length = st->crt_scales_avail_no * 2;
