				   ...
				   unsigned int offset, unsigned int size)
{
	...
	while (1) {
		...
		if (first_write) {
			/* first write to UPDATE_FW  in bootloader mode
			 * will erase the EEPROM (this takes 2 sec).
			 * See AN-1453, pag. 6
			 */
			mdelay(2000);
			first_write = false;
...
static int adm1266_program_firmware(struct adm1266_data *data)
{
	...
	write_data[1] = 0x0;
	ret = adm1266_group_cmd(data, ADM1266_GO_COMMAND, write_data, 2);
	if (ret < 0) {
	...
	write_data[1] = 0x0;
	write_data[2] = 0x0;
	ret = adm1266_group_cmd(data, ADM1266_UPDATE_FW, write_data, 3);
	if (ret < 0) {
